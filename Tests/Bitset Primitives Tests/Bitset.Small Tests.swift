...
@Suite("Bitset.Small")
struct BitsetSmallTests {

    // MARK: - Basic Operations

    @Test
    func `Init creates inline storage`() {
        let set = Bitset.Small<2>()
        #expect(!set.isSpilled)
        #expect(set.isEmpty)
        #expect(set.count == 0)
        #expect(Bitset.Small<2>.inlineCapacity == 128)
    }

    @Test
    func `Insert within inline capacity`() throws {
        var set = Bitset.Small<2>()
    ...
    func `Clear resets to inline mode`() throws {
        ...
        #expect(set.isEmpty)
        #expect(set.count == 0)
        #expect(set.capacity == Bitset.Small<2>.inlineCapacity)
    }
    ...
    func `Union in inline mode`() throws {
        var a = Bitset.Small<2>()
        try a.insert(1)
        try a.insert(2)

        var b = Bitset.Small<2>()
        try b.insert(2)
        try b.insert(3)
    ...
    func `isSubset`() throws {
        var small = Bitset.Small<2>()
        try small.insert(1)
        try small.insert(2)

        var large = Bitset.Small<2>()
        try large.insert(1)
        try large.insert(2)
        try large.insert(3)
    ...
    func `Equality mixed modes`() throws {
        ...
        // Both have same members even though different storage modes
        #expect(a == b)
    }
    ...
    func `Description inline mode`() throws {
        ...
        #expect(desc.contains("Bitset.Small"))
    ...
    func `Random operations match model`() throws {
        ...
        for _ in 0..<500 {
            ...
            switch op {
            case 0:  // insert
                let smallResult = try small.insert(value)
                let modelResult = model.insert(value).inserted
                #expect(smallResult == modelResult)

            case 1:  // remove
                let smallContains = small.contains(value)
                let modelContains = model.contains(value)
                #expect(smallContains == modelContains)
                if smallContains {
                    try small.remove(value)
                    model.remove(value)
                }

            case 2:  // contains
                #expect(small.contains(value) == model.contains(value))
